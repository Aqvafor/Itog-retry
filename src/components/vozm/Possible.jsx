    ...
    <div className="possible-container">
      <div className="content-container column">
        ...
        <p className="possible-text">
          Моменты перечислить корпоративную академию, базу знаний, учебные
          курсы, настроим систему мотивации обучения, круглосуточную поддержку.
        </p>
        <div className="possible-cards">
          {features.map((feature) => (
            <Link
              to={`/feature/${feature.imageKey}`}
              key={feature.title}
              className="possible-item"
            >
              <img
                src={imagesMap[feature.imageKey]}
                alt={feature.title}
                className="possible-image"
              />
              <TextBlock name={feature.title} text={feature.text} />
            </Link>
          ))}
        </div>
        <Ellipse
